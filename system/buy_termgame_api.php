...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_SESSION["id"])) {
        ...
        $pay_to_amount = $_POST["id"];
        $pay_to_ref1 = $_POST["ref"];
        $pay_to_company = $_POST["product"];
        $description = $_POST["description"];
        $server = isset($_POST["server"]) ? $_POST["server"] : "";

        if($server == ""){
            // ไม่มีค่า server ให้ใช้ค่าเดิม
            $pay_to_ref1 = $pay_to_ref1;
        } else {
            // มีค่า server ให้เชื่อมกับค่าเดิมด้วยช่องว่าง
            $pay_to_ref1 = $pay_to_ref1. " ". $server;
        }
            ...
            if ($q_1->rowCount() >= 1) {
                        ...
                        if (isset($TermGame["code"])) {
                            ...
                            if ($TermGame["code"] === "00000") {
                                $log = dd_q(
                                    "INSERT INTO his_purchase (ref, description, server, product, price, uid, dest_ref, type) VALUES (?,?,?,?,?,?,?,?)",
                                    [
                                        $pay_to_ref1,
                                        $description,
                                        $server,
