    ...
    if (isset($_SESSION['id'])) {
        ...
        if ($_POST['count'] != "" && intval($_POST['count']) == $_POST['count'] && $_POST['count'] > 0 && $_POST['id'] != "") {
            ...
            if ($count <= 100) {
                            ...
                            if ($pd['type'] == "1" and $find_stock->rowCount() >= $count) {
                                for ($i = 1; $i <= $count; $i++) {
                                    ...
                                    if ($find_stock->rowCount() == 1) {
                                        ...
                                        dd_return(false, "error", "ได้รับสินค้าแค่บางส่วน");
                                        break;
                                    }
                                }
                                if ($insert  and $del) {
                                    ...
                                    $sMessage = "มีรายการสั่งซื้อร้าน : " . $dt['name'] . "\n";
                                    $sMessage .= "ชื่อสินค้า : " . $pd["name"] . " \n";
                                    $sMessage .= "รายละเอียดสินค้า : " . $stock["username"] . " \n";
                                    $sMessage .= "ผู้ใช้ : " . $row_1["username"] . " \n";
                                    $sMessage .= "ราคา : " . $price_vn . " บาท \n";
                                    $chOne = curl_init();
                ...
                dd_return(false, "error",  "สามารถซื้อสูงสุดได้ 100 ครั้งต่อรอบเท่านั้น");
            }
        } else {
            dd_return(false, "error",  "สามารถซื้อสูงสุดได้ 100 ครั้งเท่านั้น");
        }
    } else {
