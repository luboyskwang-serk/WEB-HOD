<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        if (
            $_POST['id'] != "" AND $_POST['name'] != "" AND $_POST['img'] != "" AND $_POST['price'] != "" AND $_POST['des'] != "" AND $_POST['c_type'] != ""
        ) {
            $_POST['price'] = preg_replace('~\R~u', "\n", $_POST['price']);
            $q_1 = dd_q('SELECT * FROM users WHERE id = ? AND rank = 1 ', [$_SESSION['id']]);
            if ($q_1->rowCount() >= 1) {
                $insert = dd_q("UPDATE apipeam_product SET name = ? , img = ? , price_default = ? , price = ? , des = ? , c_type = ? , showitem = ? WHERE id = ? ", [
                    $_POST['name'],
                    $_POST['img'],
                    $_POST['price_default'],
                    $_POST['price'],
