<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        if ($_POST['apistatus'] AND $_POST['apipubkey'] != "" AND $_POST['apisecrets'] != "" AND $_POST['apiprice'] != "") {
            $q_1 = dd_q('SELECT * FROM users WHERE id = ? AND rank = 1 ', [$_SESSION['id']]);
            if ($q_1->rowCount() >= 1) {
                $insert = dd_q("UPDATE setup_topupapi SET 
                    status = ?,
                    pubkey = ?,
                    secrets = ?,
                    price = ?
                ...
                if($insert){
                    dd_return(true, "บันทึกข้อมูล API Games สำเร็จ");
                }else{
