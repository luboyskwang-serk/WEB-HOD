<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        if ($_POST['fname'] != "" AND $_POST['lname'] != "" AND $_POST['bnum'] != "" AND $_POST['qrcode'] != "") {
            $q_1 = dd_q('SELECT * FROM users WHERE id = ? AND rank = 1 ', [$_SESSION['id']]);
            if ($q_1->rowCount() >= 1) {
                $update = dd_q("UPDATE bank SET fname = ? , lname = ? , bnum = ? , qrcode = ?", [$_POST['fname'], $_POST['lname'], $_POST['bnum'], $_POST['qrcode']]);
                if($update){
