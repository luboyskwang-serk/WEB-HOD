...
if (isset($data['data']['cashcard'])) {
  $gtopup_data = $data['data']['cashcard'];
   ...
   const price = $(button).data('price');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
         ...
         $.ajax({
            type: 'POST',
            url: 'system/buy_cashcard_api.php',
            data: formData,
            contentType: false,
            processData: false,
         }).done(function(res) {
            ...
            if (res.status == "success") {
               Swal.fire({
                     ...
                     window.location = "?page=profile&subpage=hiscashcard";
               });
            }
            if (res.status == "fail") {
               Swal.fire({
