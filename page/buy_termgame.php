...
if (isset($data['data']['gtopup'])) {
  ...
  if (isset($_GET['game_name'])) {
    $requested_company_id = $_GET['game_name'];
      ...
      <div class="container-sm ">
         <div class="container-fluid">
            <div class="container-fluid border-2 p-0 pt-0 ">
               <div class="row container-fluid d-flex align-items-center">
                  ...
                  <!-- ชื่อเกม -->
                     ...
                     <p style="font-size:13px; color:#707070; margin-top:-1.5rem;">หากกรอก ID ผิด ร้านจะไม่รับผิดชอบทุกกรณี กรุณาเช็คให้เรียบร้อย</p>
                     ...
                     <!-- ช่องกรอก UID ID  -->
                     <a href="/assets/game/tutorial/<?= $data['company_id'] ?>.png" data-lightbox="image-1" data-title="วิธีการดู UID ของท่าน" style="text-decoration: none">
                        <div class="dedabr text-main mb-2" role="alert" style="width: 200px; font-size:14px"><i class="fa-duotone fa-circle-question fa-beat-fade"></i> ตัวอย่างข้อมูล </div>
                     </a>
                     
                     <input type="text" id="uid" class="form-control mt-1" placeholder="กรอก ID เกมของคุณ" aria-describedby="basic-addon1" style="border-radius: 4px;">
                     <div class="uid-error mu-fontre mt-2 mb-2">* กรุณากรอก ID ก่อนเลือกสินค้า *</div>
                     <?php if (!empty($data['gameservers'])): ?>
                     <br>
                     <select class="form-select mt-1 form-control" id="server">
                        <option selected>กรุณาเลือก Server เกม</option>
                        <?php foreach ($data['gameservers'] as $server): ?>
                        <option value="<?= $server['value'] ?>"><?= $server['name'] ?></option>
                        <?php endforeach; ?>
                     </select>
                     <?php else: ?>
                     <select class="form-select form-control" id="server" style="display: none;">
                        <option selected value="">กรุณาเลือก Server เกม</option>
                     </select>
                     <?php endif; ?>
                     <br>
            ...
            <div class="container col-12 td col-lg-12 ms-4">
               <p style="color:#6e7185; font-size:14px;"> เลือกรายการที่ต้องการเติม</p>
            </div>
            ...
            <div class="row container-fluid d-flex lign-items-center mb-4 ms-1" data-aos="zoom-in">
               ...
               <div class="col-12 col-lg-3 mb-1 mt-1" >
                  <div class="p-0 text-white " style="border-radius: 1vh; overflow: hidden; height: fit-content;">
                     <button class="container-fluid shop-btne p-2"  onclick="updatePrice(this)" data-code="<?= $game['price'] ?>" data-product="<?= $_GET['game_name'] ?>" data-price="<?= $free ?>" data-description="<?= htmlspecialchars($game['description']); ?>">
