...
<div class="container-sm mt-2 col-lg-10 col-10 shadow-sm p-4 mb-4" style="background-color:#0e0e0e; border-radius:1vh;">
        ...
        <table id="table" class="text-center mt-4">
            <thead class="table-dark bg-dark">
                <tr class="text-center">
                    ...
                    <th scope="col" class="ps-3 text-center" style="color:#eaac13;"> เงื่อนไข</th>
                    <th scope="col" class="ps-3 text-center" style="color:#eaac13;"> VIP</th>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td class="text-center" style="font-size:13px; color:#e8e8e8"><?php echo htmlspecialchars($row['username']); ?></td>
                        <td class="text-center" style="font-size:13px; color:#e8e8e8"><?php echo number_format($row['point']); ?></td>
                        <td class="text-center" style="font-size:13px; color:#e8e8e8"><?php echo htmlspecialchars($row['total']); ?></td>
                        <td class="text-center mt-2" style="font-size:13px; color:#e8e8e8"><?php if ($row['accept'] == 1) : ?>
                                <div class="badge alert-success"> ยอมรับเงื่อนไข </div>
                            <?php else : ?>
                                <div class="badge alert-danger"> ไม่ยอมรับเงื่อนไข </div>
                            <?php endif; ?>
                        </td>
                        <td class="text-center mt-2" style="font-size:13px; color:#e8e8e8"><?php if ($row['vip_role'] == 1) : ?>
                                <div class="badge alert-warning"> เป็นลูกค้า VIP </div>
                            <?php else : ?>
                                <div class="badge alert-secondary"> ไม่ได้เป็นลูกค้า VIP </div>
                            <?php endif; ?>
                        </td>
    ...
    <!-- <div class="d-flex justify-content-between">
        <div>
            <a class="btn btn-primary <?php if (!isset($_GET['item']) || $_GET['item'] == 1) {
                                            echo "disabled";
                                        } ?>" href="?page=backend&type=<?php echo $_GET['type']; ?>&item=<?php if (!isset($_GET['item']) || $_GET['item'] == 1) {
                                                                                                                echo "1";
                                                                                                            } else {
                                                                                                                echo $_GET['item'] - 1;
                                                                                                            } ?>" >หน้าที่แล้ว</a>
        ...
        <div>
            ...
            <a class="btn btn-primary <?php if (($current_page + 1) > $total_pages) {
                                            echo "disabled";
                                        } ?>" href="?page=backend&type=<?php echo $_GET['type']; ?>&item=<?php echo $current_page + 1; ?>">หน้าต่อไป</a>
        </div>
    </div> -->
</div>
                ...
                <div class="col-lg-10 m-cent ">
                    <div class="mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="vip" value="off">
                            <label class="form-check-label" for="vip">ลดราคาสำหรับลูกค้า VIP</label>
                        </div>
                    </div>
...
<script>
    $("#save_btn").click(function() {
        ...
        formData.append('vip', ($("#vip").prop('checked')) ? 1 : 0);
        $.ajax({
            type: 'POST',
